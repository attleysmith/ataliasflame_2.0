...
public class SpellService {
    ...
    public List<Spell> listSpells(Character character) {
        return spellRegistry.get().stream()
                .filter(spell -> !spell.getGroup().prohibitedCasteGroups.containsAll(character.getCaste().groupTags))
                ...
                .filter(spell -> !spell.getName().prohibitedCasteGroups.containsAll(character.getCaste().groupTags))
